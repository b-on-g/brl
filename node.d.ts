...
declare namespace $ {
     ...
     */
    let $mol_gap: Record<"text" | "block" | "page" | "blur" | "space" | "round" | "emoji", $mol_style_func<"var", unknown>>;
}
...
declare namespace $.$$ {
    ...
    export class $bog_brl extends $.$bog_brl {
        page(next?: PageId): PageId;
        property_id(next?: number | null): number | null;
        mobile_open(next?: boolean): boolean;
        page_index(): string;
    }
    export class $bog_brl_header extends $.$bog_brl_header {
        ...
        about_link(): any;
        contacts_link(): any;
        telegram_url(): "https://t.me/yusdanil";
        burger_toggle(): void;
    }
    export class $bog_brl_mobile_nav extends $.$bog_brl_mobile_nav {
        telegram_url(): "https://t.me/yusdanil";
        mobile_home_click(): void;
        mobile_catalog_click(): void;
        mobile_landlords_click(): void;
        mobile_about_click(): void;
        mobile_contacts_click(): void;
    }
    export class $bog_brl_home extends $.$bog_brl_home {
        catalog_link(): any;
        telegram_url(): "https://t.me/yusdanil";
        stats_rows(): $bog_brl_stat[];
        stat_number(index: number): "20%" | "150+" | "800+" | "1 день";
        stat_label(index: number): "Фиксированная комиссия" | "Объектов в базе" | "Успешных сделок" | "Среднее время подбора";
        steps_rows(): $bog_brl_step[];
        step_num(index: number): "01" | "02" | "03" | "04" | "05";
        step_icon(index: number): "🔍" | "📸" | "👥" | "📋" | "🔑";
        step_title(index: number): "Анализ объекта" | "Подготовка" | "Отбор арендаторов" | "Договор" | "Передача ключей";
        step_text(index: number): "Проверяем документы, собственника, состояние жилья" | "Реальные фото, описание, размещение в каталоге" | "Фильтрация по платёжеспособности и документам" | "Подписываем договор до передачи денег" | "Фиксируем состояние, передаём ключи";
        advantages_rows(): $bog_brl_adv[];
        adv_icon(index: number): "📋" | "💰" | "🛡️" | "🏠" | "⚡" | "🤝";
        adv_theme(index: number): "warm" | "green" | "muted";
        adv_title(index: number): "Комиссия 20% — и точка" | "Проверенные объекты" | "Договор до денег" | "Квартиры и дома" | "Быстрая связь" | "Баланс интересов";
        adv_text(index: number): "Фиксированная ставка без скрытых платежей. У большинства агентств — от 50% до 100%." | "Документы собственника, реальные фото, актуальная цена. Никаких фейков и субаренд." | "Сначала подписываем договор — потом передаём деньги. Защита для обеих сторон." | "В каталоге не только квартиры — есть частные дома для долгосрочной аренды." | "Никаких форм. Пишете в Telegram или звоните — отвечаем в течение часа." | "Мы посредник, который держит баланс и отвечает за процесс — не за одну сторону.";
    }
    export class $bog_brl_catalog extends $.$bog_brl_catalog {
        ...
        filter_price_min(next?: string): string;
        filter_price_max(next?: string): string;
        filter_type_dict(): {
            '': string;
            apartment: string;
            house: string;
        ...
        filter_district_dict(): Record<string, string>;
        apply_filters(): void;
        reset_filters(): void;
        filtered_properties(): Property[];
        results_count_value(): string;
        results_count_word(): "объект" | "объекта" | "объектов";
        property_rows(): $bog_brl_property_card[];
        property_data(id: number): Property;
        property_link(id: number): any;
        property_image(id: number): string;
        property_badge(id: number): "Дом" | "Квартира";
        property_badge_house(id: number): boolean;
        property_price(id: number): string;
        property_address(id: number): string;
        property_meta_rooms(id: number): string;
        property_meta_area(id: number): string;
        property_meta_floor(id: number): string;
    }
    export class $bog_brl_property extends $.$bog_brl_property {
        catalog_link(): any;
        telegram_url(): "https://t.me/yusdanil";
        phone_url(): "tel:+79991323141";
        phone_label(): string;
        property(): Property;
        images(): string[];
        image_index(next?: number): number;
        current_image(): string;
        next_image(): void;
        prev_image(): void;
        dot_rows(): $bog_brl_gallery_dot[];
        dot_active(index: number): boolean;
        dot_select(index: number): void;
        property_title(): string;
        property_description(): string;
        feature_rows(): $bog_brl_feature[];
        feature_text(index: number): string;
        sidebar_price_value(): string;
        sidebar_type(): "Дом · Долгосрочная аренда" | "Квартира · Долгосрочная аренда";
        sidebar_type_house(): boolean;
        spec_rows(): $bog_brl_spec[];
        specs(): {
            label: string;
            value: string;
        }[];
        spec_label(index: number): string;
        spec_value(index: number): string;
        deal_rows(): $bog_brl_deal_row[];
        deal_terms(): {
            label: string;
            value: string;
        }[];
        deal_label(index: number): string;
        deal_value(index: number): string;
    }
    export class $bog_brl_landlords extends $.$bog_brl_landlords {
        telegram_url(): "https://t.me/yusdanil";
        phone_url(): "tel:+79991323141";
        phone_label(): string;
        landlord_benefit_rows(): $bog_brl_landlord_benefit[];
        landlord_benefit_icon(index: number): "🔍" | "👥" | "📋" | "💰" | "⏱️";
        landlord_benefit_title(index: number): "Фиксированная комиссия 20%" | "Проверка арендаторов" | "Договор и юридическая защита" | "Экономия вашего времени" | "Минимум вовлечённости";
        landlord_benefit_text(index: number): "Ровно 20% от месячной аренды — за полный цикл работы. Самая низкая ставка на рынке Казани." | "Отбираем жильцов по платёжеспособности, целям аренды и документам. Вы утверждаете финального кандидата." | "Составляем типовой договор, фиксируем состояние квартиры. Всё прозрачно и законно." | "Фотосъёмка, размещение, показы и переговоры — на нас. Вам нужно только утвердить арендатора." | "Показы с вашим участием или без. Держим в курсе через Telegram на каждом шаге.";
        commission_rows(): $bog_brl_commission_item[];
        commission_text(index: number): "Профессиональная фотосъёмка и описание объекта" | "Размещение в каталоге и продвижение" | "Приём и фильтрация обращений арендаторов" | "Организация и проведение показов" | "Проверка платёжеспособности и документов" | "Составление договора аренды" | "Акт приёма-передачи с фиксацией состояния" | "Сопровождение сделки до передачи ключей";
    }
    export class $bog_brl_about extends $.$bog_brl_about {
        about_value_rows(): $bog_brl_about_value[];
        about_value_icon(index: number): "🔍" | "📋" | "⚖️";
        about_value_title(index: number): "Договор до денег" | "Баланс интересов" | "Проверка каждого объекта";
        about_value_text(index: number): "Документы, собственник, фото, цена — всё реальное" | "Сначала подпись — потом оплата. Никак иначе" | "Защищаем и арендатора, и собственника одинаково";
    }
    export class $bog_brl_contact_card extends $.$bog_brl_contact_card {
        Contact_link(): $.$mol_link;
        Contact_note(): $.$mol_text;
    }
